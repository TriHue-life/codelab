...
// ─── Sync lnums width → hl-overlay left (fixes cursor offset on mobile) ─────
function syncEditorLayout() {
  const lnW = lnEl.getBoundingClientRect().width;
  if (hlOverlay) hlOverlay.style.left = lnW + 'px';
  // Also sync line-height: read computed style from code-input
  const cs = window.getComputedStyle(ci);
  const fs = parseFloat(cs.fontSize);
  const lh = parseFloat(cs.lineHeight) || fs * 1.6;
  // Update lnums span height via CSS variable on lnEl
