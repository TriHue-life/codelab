...
(function bootstrap() {
  ...
  Events.on('auth:login', ({ user }) => {
    // Đảm bảo xoá auth-pending trước (layout mới dùng #app-shell)
    document.body.classList.remove('auth-pending');
    document.getElementById('app-shell')?.style.setProperty('visibility', '');
  ...
  window.switchTab = tab => {
    // Tab buttons: id="tab-btn-*", class="tab"
    const ids = { criteria:'tab-btn-criteria', theory:'tab-btn-theory', results:'tab-btn-results' };
    document.querySelectorAll('#grade-panel .tab').forEach(t => {
      const active = t.id === ids[tab];
      t.classList.toggle('on', active);
    });
    // Tab panels: id="tab-*", class="tc"
    document.querySelectorAll('#grade-panel .tc').forEach(p => {
