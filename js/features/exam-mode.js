...
CL.define('Features.ExamMode', () => {
  ...
  function _buildQuestionList(exam) {
    const pool = exam.bai_tap_detail || []; // array of { bai_id, nhom, diem_co_phan, bloom_level }

    // Nếu không có detail → dùng bai_tap đơn giản
    if (!pool.length) {
    ...
    // --- Chọn ngẫu nhiên theo bloom filter ---
    let selected = [];

    if (exam.bloom_filter && typeof exam.bloom_filter === 'object') {
      // Chọn số bài cụ thể từ mỗi bloom level
      const bloomGroups = {};
      ...
      for (const [lv, count] of Object.entries(exam.bloom_filter)) {
        const group = bloomGroups[lv] || [];
        const picks = _shuffle(group).slice(0, parseInt(count));
        selected.push(...picks);
      }
    } else if (exam.so_bai_random > 0) {
      // Chọn N bài ngẫu nhiên từ toàn bộ pool
  ...
  async function _submitAll() {
    ...
    // Calculate final score (average weighted)
    const totalWeight = _questions.reduce((s, q) => s + (q._weight || 1), 0);
    const diemTong    = _questions.reduce((s, q) => {
      const a = _answers[q.id];
      return s + (a?.diem_weighted || a?.diem || 0) * (q._weight || 1);
    }, 0) / totalWeight;
    const diemFinal   = Math.round(diemTong * 10) / 10;
