...
CL.define('Admin.Users', () => {
  ...
  const _stuCache = {};   // mshs → student object
  const _gvCache  = {};   // username → teacher object
        ...
        <div class="au-table-wrap">
          <table class="au-table">
            ...
            <tbody id="au-stu-tbody">
              ${students.map(s => { _stuCache[s.mshs] = s; return _stuRow(s); }).join('')}
            </tbody>
          </table>
        </div>
        ...
        <div class="au-table-wrap">
          <table class="au-table">
            ...
            <tbody id="au-gv-tbody">
              ${teachers.map(t => { _gvCache[t.username] = t; return _gvRow(t); }).join('')}
            </tbody>
          </table>
        </div>
  ...
  // ── Cache getters (avoid JSON-in-HTML-attribute escaping bugs) ──
  function _getStu(mshs) {
    return _stuCache[mshs] || { mshs };
  }

  function _getGv(username) {
    return _gvCache[username] || { username };
  }
