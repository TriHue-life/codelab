/**
 * features/admin/users.js — Admin Panel: Quản lý người dùng + điểm
 * ═══════════════════════════════════════════════════════════════
 * Tab 1: 🎓 Học sinh  — CRUD học sinh
 * Tab 2: 👨‍🏫 Giáo viên — CRUD giáo viên
 * Tab 3: ⚡ Admin     — CRUD tài khoản admin (sheet riêng)
 * Tab 4: 📊 Bảng điểm — Xem / xuất điểm toàn trường
 */
'use strict';
...
CL.define('Admin.Users', () => {
  ...
  function _auTab(btn, which) {
    ...
    const map = {
      ...
      teachers: _renderTeachers,
      admins:   _renderAdmins,
      scores:   _renderScores,
    };
    (map[which] || _renderStudents)(body);
  }
      ...
      el.innerHTML = `
        <div class="au-toolbar">
          ...
          <select class="au-select" id="au-filter-lop"
            onchange="CL.Admin.Users._filterStu(document.querySelector('.au-search')?.value||'')">
  ...
  function _stuRow(s) {
    ...
    return `<tr data-q="${Utils.escHtml((s.mshs||'')+(s.ho_ten||'').toLowerCase())}"
               data-lop="${Utils.escHtml(s.lop||'')}"
      ...
      <td class="au-actions">
        <button class="au-act-btn" title="Sửa"
          onclick="CL.Admin.Users.showStudentForm(${JSON.stringify({mshs:s.mshs,ho_ten:s.ho_ten,lop:s.lop||'',email:s.email||''})})">✏️</button>
  ...
  function _filterStu(q) {
    q = (q||'').toLowerCase();
    const lop = (document.getElementById('au-filter-lop')?.value||'').toLowerCase();
    let vis = 0;
    document.querySelectorAll('#au-stu-tbody tr').forEach(tr => {
      const matchQ   = !q   || (tr.dataset.q ||'').includes(q);
      const matchLop = !lop || (tr.dataset.lop||'').toLowerCase() === lop;
    ...
    form.innerHTML = `
      ...
      <div class="au-form-grid">
        <div class="pf-field">
          ...
          <input id="aufs-mshs" type="text" value="${Utils.escHtml(s.mshs||'')}"
            placeholder="12 chữ số" ${isEdit?'readonly':''}>
        </div>
        <div class="pf-field">
      ...
      <div style="display:flex;gap:8px;margin-top:10px">
        <button class="pf-btn-primary"
          onclick="CL.Admin.Users.saveStudent('${isEdit?s.mshs:''}')">
          💾 ${isEdit?'Lưu':'Thêm học sinh'}
        </button>
  ...
  async function saveStudent(existMshs) {
    ...
    const mshs = existMshs || document.getElementById('aufs-mshs')?.value?.trim();
    ...
    try {
      await CL.API.adminSaveUser({ role:'student', mshs, ho_ten:name, lop, email, is_new:!existMshs });
  ...
  function _gvRow(t) {
    ...
    return `<tr data-u="${Utils.escHtml(t.username||'')}"
      ...
      <td class="au-actions">
        <button class="au-act-btn" title="Sửa"
          onclick="CL.Admin.Users.showTeacherForm(${JSON.stringify({username:t.username,ho_ten:t.ho_ten,lop_phu_trach:t.lop_phu_trach||'',email:t.email||''})})">✏️</button>
        <button class="au-act-btn" title="Reset mật khẩu"
  ...
  function showTeacherForm(t) {
    ...
    form.innerHTML = `
      ...
      <div class="au-form-grid">
        ...
        <div class="pf-field">
          <label>Họ và tên <span class="pf-req">*</span></label>
          <input id="aufg-name" type="text" value="${Utils.escHtml(t.ho_ten||'')}" placeholder="Nguyễn Văn A">
        </div>
        <div class="pf-field">
          <label>Lớp phụ trách</label>
          <input id="aufg-lop" type="text" value="${Utils.escHtml(t.lop_phu_trach||'')}" placeholder="11A1, 11A2">
        </div>
        <div class="pf-field">
          <label>Email</label>
          <input id="aufg-email" type="email" value="${Utils.escHtml(t.email||'')}" placeholder="gv@thuthiem.edu.vn">
        </div>
        ${!isEdit?`<div class="pf-field">
      ...
      <div style="display:flex;gap:8px;margin-top:10px">
        <button class="pf-btn-primary"
          onclick="CL.Admin.Users.saveTeacher('${isEdit?t.username:''}')">
          💾 ${isEdit?'Lưu':'Thêm giáo viên'}
        </button>
        <button class="au-btn-cancel"
  ...
  async function saveTeacher(existUser) {
    ...
    _msg(msg,'⏳ Đang lưu...','info');
    try {
      await CL.API.adminSaveUser({ role:'teacher', username:user, ho_ten:name, lop_phu_trach:lop, email, user_role:'teacher', password:existUser?undefined:pw, is_new:!existUser });
      _msg(msg,`✅ ${existUser?'Cập nhật':'Đã thêm'} giáo viên ${user}`,'ok');
      ...
      el.innerHTML = `
        ...
        <div class="au-table-wrap">
          <table class="au-table">
            ...
            <tbody>
              ${admins.length ? admins.map(_adminRow).join('') : '<tr><td colspan="6" style="text-align:center;padding:20px;color:var(--text3)">Chưa có tài khoản admin</td></tr>'}
            </tbody>
          </table>
        </div>
  ...
  function _adminRow(a) {
    ...
    return `<tr class="${active?'':'au-row-locked'}">
      ...
      <td class="au-actions">
        ...
        <button class="au-act-btn" title="Reset mật khẩu"
          onclick="CL.Admin.Users.resetAdminPw('${Utils.escHtml(a.username||'')}')">🔑</button>
        <button class="au-act-btn au-del-btn" title="Xoá"
          onclick="CL.Admin.Users.deleteAdmin('${Utils.escHtml(a.username||'')}')">🗑</button>
      </td>
    </tr>`;
  }
  ...
  function showAdminForm(a) {
    ...
    form.innerHTML = `
      ...
      <div class="au-form-grid">
        ...
        ${!isEdit?`<div class="pf-field">
          <label>Mật khẩu <span class="pf-req">*</span></label>
          <input id="aufa-pw" type="text" placeholder="Mật khẩu mạnh">
        </div>`:''}
      </div>
      <div id="aufa-msg" class="pf-msg"></div>
      <div style="display:flex;gap:8px;margin-top:10px">
        <button class="pf-btn-primary"
          onclick="CL.Admin.Users.saveAdmin('${isEdit?a.username:''}')">
  ...
  async function resetAdminPw(username) {
    const newPw = prompt(`🔑 Mật khẩu mới cho admin "${username}":`, '');
    if (newPw === null) return;
    if (!newPw.trim() || newPw.trim().length < 4) { Toast.warn('⚠️ Mật khẩu ít nhất 4 ký tự'); return; }
    try {
      const hash = await Utils.sha256(newPw.trim());
  ...
  function _renderScoreSummary(scores) {
    ...
    scores.forEach(r => {
      const d = +r.diem || 0;
      if (d >= 9) dist['9-10']++;
      else if (d >= 7) dist['7-8']++;
      else if (d >= 5) dist['5-6']++;
      else dist['<5']++;
    });
    el.innerHTML = `
      ...
      <div class="au-score-dist">
        <div class="au-dist-bar" title="9-10" style="--pct:${(dist['9-10']/total*100).toFixed(0)}%;--clr:var(--accent2)">
          <span>9-10</span><b>${dist['9-10']}</b>
        </div>
        <div class="au-dist-bar" title="7-8" style="--pct:${(dist['7-8']/total*100).toFixed(0)}%;--clr:var(--accent)">
          <span>7-8</span><b>${dist['7-8']}</b>
        </div>
        <div class="au-dist-bar" title="5-6" style="--pct:${(dist['5-6']/total*100).toFixed(0)}%;--clr:var(--warn)">
          <span>5-6</span><b>${dist['5-6']}</b>
        </div>
        <div class="au-dist-bar" title="<5" style="--pct:${(dist['<5']/total*100).toFixed(0)}%;--clr:var(--error)">
          <span>&lt;5</span><b>${dist['<5']}</b>
        </div>
      </div>`;
  }
  ...
  function _renderScoreTable(scores) {
    ...
    if (!body) return;
    if (!scores.length) {
      body.innerHTML = '<div class="tp-empty">Chưa có dữ liệu điểm</div>';
      return;
    }
    body.innerHTML = `
      <div class="au-table-wrap">
        <table class="au-table" id="sc-table">
          <thead><tr>
            <th>MSHS</th><th>Họ tên</th><th>Lớp</th>
            <th>Bài tập</th><th>Điểm</th><th>Lần</th><th>Thời gian</th>
          </tr></thead>
          <tbody id="sc-tbody">
            ${scores.map(_scoreRow).join('')}
          </tbody>
        </table>
      </div>`;
  }
  ...
  function _scoreRow(r) {
    ...
    return `<tr
      data-q="${Utils.escHtml((r.mshs||'')+(r.ho_ten||'').toLowerCase())}"
      data-lop="${Utils.escHtml(r.lop||'')}"
      data-d="${d}"
      data-ts="${r.ts||0}"
  ...
  function _filterScores() {
    ...
    _renderScoreTable(filtered);
    _renderScoreSummary(filtered);
  }
  ...
  function _exportScoresCsv() {
    ...
    const rows   = scores.map(r =>
      [r.mshs,r.ho_ten,r.lop,(r.tieu_de||r.bai_id||'').replace(/,/g,' '),r.diem,r.lan_thu||1,r.ts||''].join(',')
    ).join('\n');
  ...
  async function resetPassword(id, role) {
    const newPw = prompt(`🔑 Mật khẩu mới cho ${role==='student'?'học sinh':'giáo viên'} "${id}":\n(Để trống = dùng ID làm mật khẩu)`);
    if (newPw === null) return;
    const pw = newPw.trim() || id;
    if (pw.length < 4) { Toast.warn('⚠️ Mật khẩu ít nhất 4 ký tự'); return; }
    try {
  ...
  return {
    render, _auTab,
    showStudentForm, saveStudent, _filterStu,
    showTeacherForm, saveTeacher, _filterGv,
