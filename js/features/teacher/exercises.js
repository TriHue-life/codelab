...
CL.define('Teacher.ExEditor', () => {
  ...
  async function syncAll() {
    if (!await Toast.confirm(
      ...
      'Quá trình mất 2–5 phút. Không đóng tab trong khi sync.'
    )) return;

    const all        = Registry.getAll();
    const BATCH      = 100;
    const totalBatch = Math.ceil(all.length / BATCH);
    let   synced     = 0;
    ...
    try {
      for (let i = 0; i < all.length; i += BATCH) {
        ...
        _updateProgress(synced, all.length, batchNum, totalBatch,
          `Batch ${batchNum}/${totalBatch}: bài ${i+1}–${Math.min(i+BATCH, all.length)}`);

        const r = await CL.API.syncExercises(batch, isFirst);
        synced += r?.synced || batch.length;

        _updateProgress(synced, all.length, batchNum, totalBatch,
          `Xong batch ${batchNum}/${totalBatch} — ${synced}/${all.length} bài`);

        await new Promise(res => setTimeout(res, 400));
      }

      _finishProgress(synced, all.length, true);
      Toast.success(`✅ Đã sync ${synced} bài tập lên Sheets!`);
    } catch(e) {
      _finishProgress(synced, all.length, false);
