...
CL.define('Teacher.ExEditor', () => {
  ...
  function _showProgress(container) {
    ...
    el.innerHTML = `
      ...
      <div class="sync-prog-wrap">
        <div class="sync-prog-bar" id="sync-prog-bar" style="width:0%"></div>
      </div>
  ...
  function _updateProgress(synced, total, batchNum, totalBatches, msg) {
    ...
    if (txt)  txt.textContent  = msg || `Batch ${batchNum}/${totalBatches} — ${synced}/${total} bài`;
  ...
  function _finishProgress(synced, total, success) {
    const box  = document.getElementById('sync-progress-box');
    const bar  = document.getElementById('sync-prog-bar');
    const txt  = document.getElementById('sync-prog-text');
    const pctEl= document.getElementById('sync-prog-pct');
    if (bar)  { bar.style.width = (success ? '100' : bar.style.width.replace('%','')) + '%';
                bar.style.background = success ? 'var(--accent2)' : 'var(--error)'; }
    if (txt)  txt.textContent = success
      ? `✅ Sync hoàn tất — ${synced} bài tập đã lên Sheets!`
      : `❌ Sync lỗi sau ${synced} bài`;
  ...
  // ── Cấu hình các tab cần sync ─────────────────────────────────
  const SYNC_TABS = [
    { tab: 'BaiTap',   label: '📋 BaiTap (metadata)'  },
    { tab: 'LyThuyet', label: '📖 LyThuyet'            },
    { tab: 'CodeMau',  label: '💻 CodeMau'             },
    { tab: 'TieuChi',  label: '📊 TieuChi'             },
    { tab: 'HuongDan', label: '⚠️ HuongDan'            },
  ];

  async function syncAll() {
    if (!await Toast.confirm(
      'Sync toàn bộ 971 bài tập lên Google Sheets?\n' +
      'Tự động sync: BaiTap + LyThuyet + CodeMau + TieuChi + HuongDan\n' +
      'Quá trình mất 3–8 phút. Không đóng tab trong khi sync.'
    )) return;

    const all        = Registry.getAll();
    const BATCH      = 100;
    const totalSteps = SYNC_TABS.length * Math.ceil(all.length / BATCH);
    let   step       = 0;
    ...
    try {
      for (const { tab, label } of SYNC_TABS) {
        ...
        for (let i = 0; i < all.length; i += BATCH) {
          ...
          _updateProgress(step, totalSteps, step, totalSteps,
            `${label} — batch ${batchNum}/${tabBatches}`);

          await CL.API.syncFull(batch, tab, isFirst);
      ...
      _finishProgress(totalSteps, totalSteps, true);
      Toast.success('✅ Sync hoàn tất! 5 tabs đã lên Google Sheets!');
    } catch(e) {
      _finishProgress(step, totalSteps, false);
