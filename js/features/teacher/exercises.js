...
CL.define('Teacher.ExEditor', () => {
  ...
  async function syncAll() {
    if (!await Toast.confirm('Sync toàn bộ 971 bài tập lên Google Sheets?\nQuá trình mất 2-5 phút, không đóng tab.')) return;
    
    const all  = Registry.getAll();
    const BATCH = 100; // Gửi 100 bài/lần để tránh timeout
    let synced  = 0;
    const btn   = document.querySelector('.tp-actions .tp-action-btn');
    ...
    try {
      for (let i = 0; i < all.length; i += BATCH) {
        ...
        const isFirst = i === 0;
        // Gửi từng batch; server append (trừ batch đầu tiên clear+write)
        const r = await CL.API.syncExercises(batch, isFirst);
        synced += r?.synced || batch.length;
        if (btn) btn.textContent = `⏳ Đang sync... ${synced}/${all.length}`;
        Toast.info(`📤 Đã sync ${synced}/${all.length} bài...`, 1500);
        await new Promise(res => setTimeout(res, 500)); // Chờ 0.5s giữa batch
      }
      Toast.success(`✅ Sync hoàn tất! ${synced} bài tập đã lên Sheets`);
      if (btn) { btn.disabled = false; btn.textContent = '🔄 Sync lên Sheets'; }
    } catch(e) {
      Toast.error('❌ Sync lỗi: ' + e.message);
      if (btn) { btn.disabled = false; btn.textContent = '🔄 Thử lại Sync'; }
    }
  }
