...
CL.define('Features.Sidebar', () => {
  ...
  function init(role) {
    ...
    // Pinned mode: expand active group (or first group if none active)
    if (_pinned) {
      const activeGroup = sb.querySelector('.sb-group.has-active');
      const firstGroup  = sb.querySelector('.sb-group');
      const toExpand    = activeGroup || firstGroup;
      if (toExpand) toExpand.classList.add('expanded');
    }
  ...
  function groupHeaderClick(gid) {
    ...
    // Collapsed/hover mode: toggle the flyout popup
    ...
    if (!isOpen) {
      ...
      _positionAndShowFlyout(gid, btn || flyout);
    }
  }
