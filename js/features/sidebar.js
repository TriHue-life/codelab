...
CL.define('Features.Sidebar', () => {
  ...
  const MENUS = {
    ...
    admin: [
      ...
      },
      {
        ...
        children:[
          { id:'config',    icon:'⚙️', label:'Cấu hình',       section:'tp:config'    },
          { id:'changelog', icon:'🕓', label:'Lịch sử phiên bản', section:'tp:changelog' },
        ]
      },
    ],
  };
  ...
  function _groupHtml(group) {
    ...
    return `
      <div class="sb-group${hasActive ? ' expanded' : ''}" data-gid="${group.id}">
        <button class="sb-group-header"
          onclick="CL.Features.Sidebar.toggleGroup('${group.id}')">
  ...
  function toggleGroup(gid) {
    const el = document.querySelector(`.sb-group[data-gid="${gid}"]`);
  ...
  function _renderUsersPanel(sub) {
    const pv = document.getElementById('panel-view');
    if (!pv) return;

    // Render admin users panel with specific tab pre-selected
    const tabMap = { student:'students', teacher:'teachers', admin:'admins', scores:'scores' };
    const tab = tabMap[sub] || 'students';

    pv.innerHTML = '<div class="pv-loading">⏳ Đang tải...</div>';
    CL.Admin.Users?.render(pv).then?.(() => {
      setTimeout(() => CL.Admin.Users?._auTab(null, tab), 100);
    });
    // For sync render
    if (CL.Admin.Users?.render) {
      CL.Admin.Users.render(pv);
      setTimeout(() => CL.Admin.Users?._auTab(null, tab), 200);
    }
  }
