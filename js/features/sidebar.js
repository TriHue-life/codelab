...
CL.define('Features.Sidebar', () => {
    ...
    return `
      <div class="sb-group${hasActive ? ' has-active' : ''}" data-gid="${group.id}">
        <button class="sb-group-header${hasActive ? ' has-active' : ''}"
          aria-haspopup="true"
          onmouseenter="CL.Features.Sidebar._showFlyout('${group.id}', this)" onmouseleave="CL.Features.Sidebar._hideFlyout('${group.id}')"
  ...
  function groupHeaderClick(gid) {
    ...
    if (isPinned) {
      // Pinned mode: toggle expand/collapse the group inline
      const grpEl = sidebar?.querySelector(`.sb-group[data-gid="${gid}"]`);
      if (grpEl) grpEl.classList.toggle('expanded');
      return;
    }
    // Narrow mode: flyout shown on hover, click navigates first child
    const groups = MENUS[_role] || MENUS.student;
  ...
  function _showFlyout(gid, btn) {
    // Position flyout to the RIGHT of sidebar
    const sb = document.getElementById('sidebar');
