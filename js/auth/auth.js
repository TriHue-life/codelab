...
CL.define('Auth.UI', () => {
  ...
  function init(onReady) {
    ...
    if (existing) {
      // Session còn hợp lệ (F5, mở lại tab) → khôi phục không cần login lại
      document.body.classList.remove('auth-pending');  // Hiện workspace
      _applyRole(existing);
      CL.Events?.emit('auth:login', { user: existing }); // Trigger panel/mode
      onReady(existing);
    } else {
      _renderLoginScreen();
    }
  ...
  function _renderLoginScreen() {
    ...
    ov.innerHTML = `
      <div class="auth-card">
        ...
        <div id="form-student" class="auth-form" role="tabpanel">
          <!-- Hidden fake fields to trick browser autocomplete away from real fields -->
          ...
          <div class="auth-field">
            ...
            <div class="auth-input-wrap">
              ...
              <input id="in-user" type="text" placeholder="username"
                autocomplete="section-teacher username" spellcheck="false"
          ...
          <div class="auth-field">
            ...
            <div class="auth-input-wrap">
              ...
              <input id="in-pass" type="password"
                placeholder="Nhập mật khẩu"
                autocomplete="section-teacher current-password"
    ...
    // Xoá readonly sau 500ms để browser không kịp autofill
    setTimeout(() => {
      ['in-mshs','in-hs-pw'].forEach(id => {
  ...
  function _switchTab(role) {
    ...
    // Reset student fields khi chuyển tab - tránh browser autofill nhầm
    if (role === 'student') {
      setTimeout(() => {
        const m = document.getElementById('in-mshs');
        const p = document.getElementById('in-hs-pw');
        if (m) { m.value = ''; m.setAttribute('readonly', ''); }
        if (p) { p.value = ''; p.setAttribute('readonly', ''); }
      }, 50);
    }
    setTimeout(() => {
