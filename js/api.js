...
CL.define('API', () => {
  ...
  async function _post(body) { return Http.post(_url(), body); }
  ...
  async function syncExercises(exs)       { return _post({ action: 'syncExercises', token: _tok(), exercises: exs }); }
  ...
  const facade = { setUrl, getUrl, isReady, getToken: _tok,
    login, logout, changePassword, getExerciseDetail,
    submitScore, getScores, getHistory,
    logViolation, getViolations, logAccess, getAccessLog,
    getExams, saveExam, deleteExam, setExamStatus,
    saveLyThuyet, saveCodeMau, saveTieuChi, saveHuongDan, syncExercises,
