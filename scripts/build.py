...
# ── Auto-packaging: tạo zip theo phiên bản ─────────────────────
import json, zipfile as zf_mod, re as re_mod

def _get_version():
    cfg = (ROOT / 'js/core/config.js').read_text(encoding='utf-8')
    m   = re_mod.search(r"APP_VERSION\s*:\s*'([^']+)'", cfg)
    return m.group(1) if m else '4.0.0'

def _make_zip(version):
    zip_path = ROOT / f'CodeLab-v{version}.zip'
    SKIP_DIRS = {'.git', 'node_modules', '__pycache__', 'dist'}
    SKIP_EXT  = {'.pyc', '.bak', '.log', '.zip'}

    with zf_mod.ZipFile(zip_path, 'w', zf_mod.ZIP_DEFLATED) as z:
        for fp in sorted(ROOT.rglob('*')):
            ...
            z.write(fp, f'python-grader/{rel}')

    kb = zip_path.stat().st_size // 1024
    n  = len([x for x in zf_mod.ZipFile(zip_path).namelist()])
    print(f'📦 CodeLab-v{version}.zip — {kb}KB, {n} files → {zip_path}')
    return zip_path
