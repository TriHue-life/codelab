...
function handleUploadImage(body) {
  ...
  try {
    ...
    // Find or create CodeLab/Images folder
    let folder;
    ...
    folder = imgFolders.hasNext() ? imgFolders.next() : cl.createFolder('Images');

    // Create file + set public viewing
    const file = folder.createFile(blob);
    file.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);

    // Return direct image URL (works for embedding in <img src="...">)
    const fileId = file.getId();
    const url    = `https://drive.google.com/uc?export=view&id=${fileId}`;

    return ok({ url, fileId, filename: file.getName() });
...
function handleSaveExerciseContent(body) {
  ...
  if (!bai_id || !field || !['desc', 'theory'].includes(field)) {
  ...
  // Invalidate cache
  CacheService.getScriptCache().remove('content_' + bai_id + '_' + field);
  ...
  if (role === 'student') {
    ...
    if (is_new) {
      ...
      for (let i = 1; i < vals.length; i++) {
        ...
        if (pw_col     >= 0 && password)                sheet.getRange(i+1, pw_col+1).setValue(hashPwd(password));
        break;
      }
