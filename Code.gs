...
function requireTeacher(body) {
  ...
  if (sess.role !== 'teacher') throw new Error('Chỉ giáo viên mới có quyền thực hiện thao tác này.');
  return sess;
}
...
function handleLogin(body) {
  ...
  if (role === 'teacher') {
    ...
    const token = createToken({ role:'teacher', username:u, name:found.ho_ten||u, email:found.email||'', lop:found.lop_phu_trach||'' });
    _logAccess(u, found.ho_ten||u, '', 'teacher', 'login', '');
    return ok({ token, name:found.ho_ten||u, lop:found.lop_phu_trach||'', email:found.email||'' });
  }
...
function handleSyncExercises(body) {
  ...
  const rows = exercises.map(e => BAITAP_H.map(h => {
    const map = {
      ...
      tieu_de: e.title, mo_ta: (e.desc||'').replace(/<[^>]+>/g,' ').substring(0,500),
      type: e.type||'python', updated_at: nowISO()
    };
    return map[h] ?? '';
  }));
  if (rows.length) sheet.getRange(2, 1, rows.length, BAITAP_H.length).setValues(rows);
...
function handleCreateTables(body) {
  ...
  ensureTab('TAIKHOAN', 'GiaoVien', ['username','password_hash','ho_ten','email','lop_phu_trach','role','active']);
  ensureTab('TAIKHOAN', 'HocSinh',  ['mshs','password_hash','ho_ten','email','lop','ngay_sinh','active']);
...
function handleAutoSetup(body) {
  ...
  try {
    ...
    if (existGV.length <= 1) {
      const adminUser = body.admin_username || 'giaovien';
      const adminPwd  = body.admin_password || 'thuthiem@2025';
      gvSheet.appendRow([adminUser, hashPwd(adminPwd), body.admin_name||'Giáo viên Admin', body.admin_email||'', '', 'teacher', true]);
      log.push(`👤 Tạo tài khoản GV: ${adminUser} / ${adminPwd}`);
    }
